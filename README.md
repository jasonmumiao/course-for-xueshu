...
学术交流月的课程很多，培养方案里每位同学一次只能选择两门。早期选课界面只展示当前已选两门课的选课情况和剩余人数，所以很难直接比较所有课程。

这会带来一个很麻烦的问题：如果想知道其他课程还有没有余量，只能先退掉已有课程，再换选其他课程，进入选课界面查看，再继续重复这个过程。对想综合比较所有课程余量、上课时间和冲突情况的同学来说，这个流程非常低效，也容易把原本想保留的课程弄乱。

这个看板就是为了解决这个信息不透明的问题：通过一个书签脚本在学校页面同源运行，直接读取学校 `kbcx` 课程余量页面里的 A 类和 B 类学术交流月课程数据，并把所有结果汇总到公开看板。脚本不会退选、加选或修改培养方案。
...
3. 把弹窗里的蓝色书签按钮拖到浏览器书签栏。
4. 点击 `我已放好书签，跳转课程余量页`。
5. 在学校页面登录后，点击刚拖好的书签。
6. 等进度面板跑完。脚本会读取 `kbcx` 页面、回传结果，并跳回看板。

旧书签不会自动更新。每次改了刷新范围，都需要从弹窗重新拖一次书签。
...
- 刷新脚本运行在 `yjsjy.uestc.edu.cn` 学校页面内，依靠浏览器自己的登录态读取 `kbcx` 课程余量页面。
...
`REQUEST_DELAY_MS` 只供旧版后端代请求备用接口使用；当前书签刷新流程直接读取 `kbcx` 页面，不再需要轮换课程间隔。
